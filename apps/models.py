...
class Orders(models.Model):
    ...
    is_delete = models.BooleanField(verbose_name='是否删除',default=False)
    total_price = models.FloatField(verbose_name='总计')
...
class Users(models.Model):
    ...
    userEmail = models.CharField(max_length=50,verbose_name='用户邮箱',default='enter@youremail.com')
    status = models.BooleanField(verbose_name='用户状态',default=False)
    is_delete = models.BooleanField(verbose_name='用户是否删除',default=False)

    def toDict(self):
        return {'user_id':self.user_id,'status':self.status, 'userName':self.userName,'userPhoneNumber':self.userPhoneNumber,'userEmail':self.userEmail}
    class Meta:
        db_table='users'
class AdminUsers(models.Model):
    ...
    userPhoneNumber = models.CharField(max_length=11,verbose_name='用户手机号')
    userEmail = models.CharField(max_length=50,verbose_name='用户邮箱',default='enter@youremail.com')
    status = models.BooleanField(verbose_name='用户状态',default=True)
    is_delete = models.BooleanField(verbose_name='用户是否删除',default=False)

    def toDict(self):
        return {'id':self.id,'status':self.status, 'userName':self.userName,'userPhoneNumber':self.userPhoneNumber,'userEmail':self.userEmail}
    class Meta:
        db_table='adminuser'
